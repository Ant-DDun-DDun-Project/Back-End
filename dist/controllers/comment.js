...
class commentControllers {
    ...
    async postComment(req, res, next) {
        try {
            const { comment, date } = await joi_1.default.postCommentSchema.validateAsync(req.body); //req.body로 댓글 정보(내용,작성날짜)를 받는다.
            ...
            const targetComment = await models_1.Comment.findOne({
                where: {
                    user,
                    multi: multi_id,
                    comment,
    ...
    async editComment(req, res, next) {
        try {
            const { comment, editedDate } = await joi_1.default.editCommentSchema.validateAsync(req.body); //req.body로 수정할 댓글 정보(내용, 수정날짜)를 받는다.
            const { multi_id, comment_id } = req.params; //req.params로 해당 댓글이 달린 게시물의 고유id와 댓글의 고유id을 받는다.
            const user = res.locals.user; //현재 로그인한 user의 고유id
            if (await models_1.Comment.findOne({ where: { user, multi: multi_id, id: comment_id } })) {
                //해당 댓글의 작성자가 로그인 한 user가 맞으면
                await models_1.Comment.update({
                    comment,
                    editedDate,
                    edited: true,
                }, { where: { user, multi: Number(multi_id), id: Number(comment_id) } }); //해당 댓글 수정
                const [targetComment, nickname] = await Promise.all([
                    //Promise.all로 update된 해당 댓글과 user의 닉네임을 병렬적으로 가져와서 각각 변수로 지정
                    models_1.Comment.findOne({
                ...
                const newComment = Object.assign(nickname, targetComment); //두 객체(닉네임,해당 댓글 정보)를 하나로 합침
                res.status(200).json({
                    success: true,
                    newComment,
    ...
    async deleteComment(req, res, next) {
        try {
            ...
            if (await models_1.Comment.findOne({ where: { user, multi: multi_id, id: comment_id } })) {
                ...
                await models_1.Comment.update({
                    deleted: true,
                }, { where: { user, multi: multi_id, id: comment_id } }); //해당 댓글의 삭제상태를 true로 변경
                const [targetComment, nickname] = await Promise.all([
    ...
    async likeComment(req, res, next) {
        try {
            ...
            if (likeExist) {
                ...
                //좋아요를 한 기록이 없으면
                await models_1.CommentLike.create({ user, comment: Number(comment_id), multi: Number(multi_id) }); //좋아요 기록 생성
                const likeCnt = await models_1.CommentLike.count({
                    where: { comment: comment_id, multi: multi_id },
                }); //해당 댓글의 좋아요 갯수 확인
                await models_1.Comment.update({ likeCnt }, { where: { id: Number(comment_id), multi: Number(multi_id) } }); //댓글 좋아요 수 update
