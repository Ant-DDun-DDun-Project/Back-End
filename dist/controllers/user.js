...
class userControllers {
    constructor() {
        ...
        this.signUp = async (req, res, next) => {
            try {
                const { userId, nickname, pw, confirmPw, ageGroup, } = await joi_1.default.signUpSchema.validateAsync(req.body); //req.body로 user정보(아이디, 닉네임, 비밀번호, 비밀번호확인, 연령대)를 받는다.
                if ((0, password_validation_1.validatePassword)(pw, confirmPw)) {
                    ...
                    const [encryptedPw, userExist] = await Promise.all([
                        ...
                        bcrypt.hash(pw, salt),
                        models_1.User.findOne({ where: { userId } }),
                    ]);
                    if (!userExist) {
                        //user가 존재하지 않으면
        ...
        this.login = async (req, res, next) => {
            try {
                const { userId, pw } = await joi_1.default.loginSchema.validateAsync(req.body); //req.body로 user정보(아이디,비밀번호)를 받는다
                const userData = await models_1.User.findOne({ where: { userId } }); //user의 아이디로 user 유무를 찾는다.
                ...
                if (!userData || !pwCheck) {
                    ...
                    const userId = userData.id; //user의 아이디
                    const token = (0, create_token_1.createToken)(userId); //user의 아이디를 페이로드에 담은 토큰을 만든다.
        ...
        // 로그인 상태 확인인
        this.checkLoginStatus = async (req, res, next) => {
            try {
                ...
                if (user === 13) {
                    ...
                    //user가 있으면
                    const loginUser = await models_1.User.findOne({ where: { id: user } }); //user의 고유id로 로그인한 user의 데이터를 불러온다
                    if (!loginUser) {
                        //해당 id를 가진 user가 없으면
        ...
        //아이디 중복체크
        this.CheckDuplicatedId = async (req, res, next) => {
            try {
                const { userId } = await joi_1.default.duplicatedIdSchema.validateAsync(req.body); //req.body로 user의 아이디를 받는다.
                if (await models_1.User.findOne({ where: { userId } })) {
                    //이미 DB에 존재하는 아이디일 경우
        ...
        //닉네임 중복 체크에 대한 함수
        this.CheckDuplicatedNick = async (req, res, next) => {
            try {
                const { nickname } = await joi_1.default.duplicatedNickSchema.validateAsync(req.body); //req.body로 닉네임을 받아온다.
                if (await models_1.User.findOne({ where: { nickname } })) {
                    //이미 DB에 존재하는 닉네임일 경우
