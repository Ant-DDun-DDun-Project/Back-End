...
class MultiQuery {
  ...
  getComment = (user, multi_id) => {
    ...
    FROM comments WHERE multi = ${multi_id} ORDER BY date
    `;
  };
  ...
  getChildComment = (user, multi_id) => {
    ...
    (SELECT user FROM commentlikes WHERE user = ${user} AND commentlikes.childComment = childcomments.id) AS liked
    FROM childcomments WHERE multi = ${multi_id} ORDER BY date
    `;
  };
}
...
class SearchQuery {
  searchEither = (keyword) => {
    return `
    SELECT eitherId, user, title, date, editedDate, completed, likeCnt FROM either 
