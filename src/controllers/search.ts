  ...
  public searchPosts = async (
    ...
    try {
      ...
      const keywords: string[] = keyword
        .trim()
        .replace(/\s\s+/gi, ' ')
        .split(' '); //공백 제거 (trim으로 앞뒤 공백 제거, replace를 통해서 2번 연속으로 공백이 되어있는 것을 1번공백으로 적용.) 후 공백으로 keyword를 나눔
      if (!keyword.length) {
        //keyword가 없으면
      ...
      for (let word of keywords) {
        ...
        const [searchedEither, searchedMulti]: [EitherList[], MultiList[]] =
          await Promise.all([
      ...
      let posts: any = searchedPosts.flat().sort((b, a) => {
        return a.date < b.date ? -1 : a.date > b.date ? 1 : 0;
      }); //다차원 배열을 1차원 배열로 만든 후 날짜순으로 정렬
      const eitherId: any[] = [];
      const multiId: any[] = [];

      posts = posts.filter((value: any, i: any) => {
        if (eitherId.findIndex(value.eitherId) === -1) {
          eitherId.push(value.eitherId);
          return true;
        } else if (multiId.findIndex(value, multiId) === -1) {
          multiId.push(value.multiId);
          return true;
        } else {
          return false;
        }
