...
import { MultiInfo, MultiReq, TargetMultiInfo } from '../interfaces/post';
...
class multiControllers {
  ...
  public getMulti = async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (multi_id === 'all') {
        ...
        const multi: MultiInfo[] = await sequelize.query(multiQuery.getMulti(user), {
          type: QueryTypes.SELECT,
        }); //객관식 포스팅을 가져옴
        res.status(200).json({ success: true, multi }); //status code 200, success:true, 객관식 포스팅을 보낸다
      } else {
        //상세페이지나 특정페이지를 보고난 후 main view로 갔을때
        const unsortedMulti: MultiInfo[] = await sequelize.query(multiQuery.getMulti(user), {
          type: QueryTypes.SELECT,
        }); //객관식 포스팅을 가져옴
  ...
  public getIngMulti = async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (multi_id === 'all') {
        //뒤로가기나 해당 게시물 클릭을 통해서 main view로 온 것이 아닐때
        const multi: MultiInfo[] = await sequelize.query(multiQuery.getIngMulti(user), {
          type: QueryTypes.SELECT,
        }); //객관식 포스팅을 가져옴
        res.status(200).json({ success: true, multi }); //status code 200, success:true, 객관식 포스팅을 보낸다
      } else {
        //상세페이지나 특정페이지를 보고난 후 main view로 갔을때
        const unsortedMulti: MultiInfo[] = await sequelize.query(multiQuery.getIngMulti(user), {
          type: QueryTypes.SELECT,
        }); //객관식 포스팅을 가져옴
        const multi = sortMulti(unsortedMulti, multi_id); //해당 고유id를 가진 포스팅이 가장 먼저 오도록 정렬
        res.status(200).json({ success: true, multi }); //status code 200, success:true, 객관식 포스팅을 보낸다.
  ...
  public getCompleteMulti = async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (multi_id === 'all') {
        ...
        //상세페이지나 특정페이지를 보고난 후 main view로 갔을때
        const unsortedMulti: MultiInfo[] = await sequelize.query(
  ...
  public getTargetMulti = async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const user: number = res.locals.user; //현재 로그인한 user의 고유id
      const multi: TargetMultiInfo[] = await sequelize.query(multiQuery.getTargetMulti(user, multi_id), {
        type: QueryTypes.SELECT,
      }); //특정 객관식 포스팅 데이터를 가져옴
      if (multi.length > 0) {
        // 객관식 게시글이 존재하는 경우
        const [comment, childComment]: CommentInfo[][] = await Promise.all([
  ...
  public postMulti = async (req: Request, res: Response, next: NextFunction) => {
    try {
      const {
        ...
        contentD,
        contentE,
      }: MultiReq = await joi.multiSchema.validateAsync(req.body); //req.body로 객관식 데이터(제목,내용, 선택지 A,B,C,D,E)를 받아온다.
  ...
  public editMulti = async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const {
        ...
        contentD,
        contentE,
      }: MultiReq = await joi.editMultiSchema.validateAsync(req.body); //req.body로 객관식 데이터(제목,내용, 선택지 A,B,C,D,E, 수정날짜)를 받아온다.
      const editedDate: string = moment().format('YYYY-MM-DD HH:mm:ss'); //수정날짜
      const user: number = res.locals.user;
      const [userCheck, voted]: [MultiModel | null, VoteModel | null] = await Promise.all([
  ...
  public deleteMulti = async (req: Request, res: Response, next: NextFunction) => {
    try {
      const { multi_id } = req.params; //req.params로 해당 게시물 고유id를 받아옴
      const user: number = res.locals.user; //현재 로그인한 user의 고유id
      const userCheck: MultiModel | null = await Multi.findOne({ where: { multiId: multi_id, user } }); //user가 해당 게시물을 작성했는지 확인
      if (userCheck) {
        //작성자가 맞고
  ...
  public likeMulti = async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const user: number = res.locals.user;
      const alreadyLike: LikeModel | null = await Like.findOne({ where: { multi: multi_id, user } });
      if (alreadyLike) {
        res.status(400).json({ success: false });
      } else {
  ...
  public voteMulti = async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const alreadyVote: VoteModel | null = await Vote.findOne({ where: { user, multi: multi_id } }); //이미 투표했는지 확인
      if (alreadyVote) {
        //이미 투표했으면
  ...
  public completeMulti = async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const userCheck: MultiModel | null = await Multi.findOne({
