...
export const sortMulti = (unsortedPosts: MultiInfo[], posts_id: number | string): MultiInfo[] => {
  ...
  const multi: MultiInfo[] = []; //뒤에 붙일 객관식 포스팅을 담을 배열 생성
...
export const sortEither = (unsortedPosts: EitherInfo[], posts_id: number | string): EitherInfo[] => {
  //정렬되지 않은 포스팅과 특정 찬반 고유id를 인자로 받음
  const either: EitherInfo[] = []; //뒤에 붙일 찬반 포스팅을 담을 배열 생성
