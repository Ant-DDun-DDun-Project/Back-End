...
class eitherControllers {
  ...
  public postEither = async (req: Request, res: Response, next: NextFunction) => {
    try {
      const { title, contentA, contentB, date }: {
        title: string, contentA: string, contentB: string, date: string
  ...
  public getEither = async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (either_id === 'all') {
        ...
        const either: object[] = await sequelize.query(eitherQuery.getEither(user), {
  ...
  public getIngEither = async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (either_id === 'all') {
        //뒤로가기나 해당 게시물 클릭을 통해서 main view로 온 것이 아닐때
        const either: object[] = await sequelize.query(eitherQuery.getIngEither(user), {
          type: QueryTypes.SELECT,
        }); //찬반투표 진행중 게시물
        res.status(200).json({
          success: true,
  ...
  public getCompleteEither = async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (either_id === 'all') {
        //뒤로가기나 해당 게시물 클릭을 통해서 main view로 온 것이 아닐때
        const either: object[] = await sequelize.query(eitherQuery.getCompleteEither(user), {
          type: QueryTypes.SELECT,
        }); //찬반투표 종료 게시물
        res.status(200).json({
          success: true,
  ...
  public editEither = async (req: Request, res: Response, next: NextFunction) => {
    try {
      const { title, contentA, contentB, editedDate }: {
        title: string, contentA: string, contentB: string, editedDate: string
